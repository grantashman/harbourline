...
- No billing, payment, financial calculation, bank, contacts, location, camera,
  microphone, calendar-write, or notification plugin is included.
...
parser or written to logs. The native projects declare HTTPS app/universal-link
callbacks for the root, support, and export paths. Auth fragments are forwarded
only when their fields match the Supabase implicit-flow response shape; hosted
association files and final Supabase session handoff still require the
downstream shell-hardening task.
