...
export function parseApprovedAuthReturn(query: URLSearchParams): ApprovedAuthReturn | null {
  ...
  if (!isAuthNavigation && !isGoogleAuthNavigation && uniqueKeys.size !== 1 && !isBillingAccount && !isAuthWithState) return null;

  const allowedKeys = new Set(["account", "provider", "recovery", "billing", "calendar", "state"]);
  ...
  if (isAuthIntent && !isAuthNavigation && !isGoogleAuthNavigation && !isBillingAccount && !query.has("state")) return null;
